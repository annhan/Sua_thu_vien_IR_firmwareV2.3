	 ...
	 // DEBUG_OUTPUT.print(plainBuf);
      if (contentLength > 0) {
        ...
        if(isEncoded){
		  ...
		  //unsigned int dd=plainBuf.length();
		  //DEBUG_OUTPUT.println(dd);
		  if (decodedLen != 0){
			  //DEBUG_OUTPUT.println("MHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHH");
          memcpy(plainBuf, decoded.c_str(), decodedLen);
          plainBuf[decodedLen] = 0;
		  }
		  else {
			  String decoded1=plainBuf;
			  decoded1.replace("+"," ");
			  decoded1.replace("%2C",",");
			  //DEBUG_OUTPUT.println("AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAa");
			  //DEBUG_OUTPUT.println(decoded1);
			  size_t decodedLen1 = decoded1.length();
			  memcpy(plainBuf, decoded1.c_str(), decodedLen1);
			  plainBuf[decodedLen1] = 0;
	      }
		  searchStr += plainBuf;
		  //_parseArguments(searchStr);
        }
		 _parseArguments(plainBuf);
		
		//DEBUG_OUTPUT.print("HHHHHHHHHHHHHHHHH ");
		//_parseArguments(plainBuf);
        //_parseArguments(searchStr);
        if(!isEncoded){
...
{	
	...
	//DEBUG_OUTPUT.print("Chieu dai ");
	//DEBUG_OUTPUT.println(len);
	unsigned int i = 0;
	//decoded.replace("%2C",",");
	//text=decoded;
	//decoded="";
	//if (len > 3000){DEBUG_OUTPUT.println(text);
	...
	//	return decoded;
	//}
	//DEBUG_OUTPUT.println("No hon 3000 :");
	while (i < len)
	{	
		char decodedChar;
